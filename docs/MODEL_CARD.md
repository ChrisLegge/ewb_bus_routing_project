...
### What R² = 0.945 does — and does not — mean

The test targets are generated by this project's own demand pipeline, so R² here
measures **pipeline capability** — that this architecture can learn demand of this
complexity from features of this kind, with no leakage under a temporal split — **not
real-world forecast accuracy**. We have never claimed otherwise. What connects the
model to reality is a chain of independent anchors, weakest link disclosed last: the
